...
trendArray1<-array(NA,dim=c(l,m))
trendArray2<-array(NA,dim=c(l,m))
trendArrayDiff<-array(NA,dim=c(l,m))
A <- matrix(data=0, nrow=n, ncol=3)
...
# Convert cm to mm when calculating the trends
    junk <- lm(data[j]*10 ~ daytable$jd[j])
# Convert from mm/day to mm/yr
    trendArray[i] <- junk$coef[2]*365

# Trends for two halves of timeseries
    Atemp <- A[j,]
# Check that there is data in both halves
    if (all(Atemp[,2]==0)||all(Atemp[,3]==0)){
      trendArray1[i] <- NA
      trendArray2[i] <- NA
    } else {
      junk <- solve(t(Atemp)%*%Atemp)%*%t(Atemp)%*%(data[j]*10)
      trendArray1[i] <- junk[2,1]
      trendArray2[i] <- junk[3,1]
    }
...
zonalMeanSE <- vector(mode="numeric", length=m)
for ( i in 1:m ){
# Only use values with a reasonable rate (+/-50mm/yr)
  ...
  len <- length(is.finite(trendArray[j,i]))
  zonalMeanSE[i] <- sum(trendArray[j,i],na.rm=TRUE)/sqrt(len)
}
...
zonalAtlanticMeanSE <- vector(mode="numeric", length=m)
zonalPacificMeanSE <- vector(mode="numeric", length=m)
for ( i in 1:m ){
  junk <- trendArray[pacLon,i]
# Only use values with a reasonable rate (+/-50mm/yr)
  j<-intersect(which(junk>=-50),which(junk<=50))
  zonalPacificMean[i] <- mean(junk[j],na.rm=TRUE)
  len <- length(is.finite(junk[j]))
  zonalPacificMeanSE[i] <- sum(junk[j],na.rm=TRUE)/sqrt(len)

  junk <- trendArray[atlLon,i]
  j<-intersect(which(junk>=-50),which(junk<=50))
  zonalAtlanticMean[i] <- mean(junk[j],na.rm=TRUE)
  len <- length(is.finite(junk[j]))
  zonalAtlanticMeanSE[i] <- sum(junk[j],na.rm=TRUE)/sqrt(len)
}
