...
s177ScandinaviaS <- s177PgrCorrectedRates[56,j]
s177Scandinavia <- s177PgrCorrectedRates[1:55,j]
s177ScandinaviaLat <- stns177Lat[j]
s177ScandinaviaLon <- stns177LonNeg[j]
s177ScandinaviaNames <- stns177$sname[j]

s177ScandinaviaMean <- array(NA,dim=c(56,1))
s177ScandinaviaSD <- array(NA,dim=c(56,1))
s177ScandinaviaSE <- array(NA,dim=c(56,1))
for ( i in 1:55){
...
s177ScandinaviaMean[56] <- mean(s177ScandinaviaS, na.rm=TRUE)
s177ScandinaviaSD[56] <- sd(s177ScandinaviaS, na.rm=TRUE)
s177ScandinaviaSE[56] <- s177ScandinaviaSD[56]/
...
s177NEuropeS <- s177PgrCorrectedRates[56,j]
s177NEurope <- s177PgrCorrectedRates[1:55,j]
s177NEuropeLat <- stns177Lat[j]
s177NEuropeLon <- stns177LonNeg[j]
s177NEuropeNames <- stns177$sname[j]

s177NEuropeMean <- array(NA,dim=c(56,1))
s177NEuropeSD <- array(NA,dim=c(56,1))
s177NEuropeSE <- array(NA,dim=c(56,1))
for ( i in 1:55){
...
s177NEuropeMean[56] <- mean(s177NEuropeS, na.rm=TRUE)
s177NEuropeSD[56] <- sd(s177NEuropeS, na.rm=TRUE)
s177NEuropeSE[56] <- s177NEuropeSD[56]/
...
s177EAtlanticS <- s177PgrCorrectedRates[56,j]
s177EAtlantic <- s177PgrCorrectedRates[1:55,j]
s177EAtlanticLat <- stns177Lat[j]
s177EAtlanticLon <- stns177LonNeg[j]
s177EAtlanticNames <- stns177$sname[j]

s177EAtlanticMean <- array(NA,dim=c(56,1))
s177EAtlanticSD <- array(NA,dim=c(56,1))
s177EAtlanticSE <- array(NA,dim=c(56,1))
for ( i in 1:55){
...
s177EAtlanticMean[56] <- mean(s177EAtlanticS, na.rm=TRUE)
s177EAtlanticSD[56] <- sd(s177EAtlanticS, na.rm=TRUE)
s177EAtlanticSE[56] <- s177EAtlanticSD[56]/
...
s177MediterraneanS <- s177PgrCorrectedRates[56,j]
s177Mediterranean <- s177PgrCorrectedRates[1:55,j]
s177MediterraneanLat <- stns177Lat[j]
s177MediterraneanLon <- stns177LonNeg[j]
s177MediterraneanNames <- stns177$sname[j]

s177MediterraneanMean <- array(NA,dim=c(56,1))
s177MediterraneanSD <- array(NA,dim=c(56,1))
s177MediterraneanSE <- array(NA,dim=c(56,1))
for ( i in 1:55){
...
s177MediterraneanMean[56] <- mean(s177MediterraneanS, na.rm=TRUE)
s177MediterraneanSD[56] <- sd(s177MediterraneanS, na.rm=TRUE)
s177MediterraneanSE[56] <- s177MediterraneanSD[56]/
...
s177SEAsiaS <- s177PgrCorrectedRates[56,j]
s177SEAsia <- s177PgrCorrectedRates[1:55,j]
s177SEAsiaLat <- stns177Lat[j]
s177SEAsiaLon <- stns177LonNeg[j]
s177SEAsiaNames <- stns177$sname[j]

s177SEAsiaMean <- array(NA,dim=c(56,1))
s177SEAsiaSD <- array(NA,dim=c(56,1))
s177SEAsiaSE <- array(NA,dim=c(56,1))
for ( i in 1:55){
...
s177SEAsiaMean[56] <- mean(s177SEAsiaS, na.rm=TRUE)
s177SEAsiaSD[56] <- sd(s177SEAsiaS, na.rm=TRUE)
s177SEAsiaSE[56] <- s177SEAsiaSD[56]/sqrt(length(which(is.finite(s177SEAsiaS))))
...
s177AustralasiaS <- s177PgrCorrectedRates[56,j]
s177Australasia <- s177PgrCorrectedRates[1:55,j]
s177AustralasiaLat <- stns177Lat[j]
s177AustralasiaLon <- stns177LonNeg[j]
s177AustralasiaNames <- stns177$sname[j]

s177AustralasiaMean <- array(NA,dim=c(56,1))
s177AustralasiaSD <- array(NA,dim=c(56,1))
s177AustralasiaSE <- array(NA,dim=c(56,1))
for ( i in 1:55){
...
s177AustralasiaMean[56] <- mean(s177AustralasiaS, na.rm=TRUE)
s177AustralasiaSD[56] <- sd(s177AustralasiaS, na.rm=TRUE)
s177AustralasiaSE[56] <- s177AustralasiaSD[56]/
...
s177PacificIslandsS <- s177PgrCorrectedRates[56,j]
s177PacificIslands <- s177PgrCorrectedRates[1:55,j]
s177PacificIslandsLat <- stns177Lat[j]
s177PacificIslandsLon <- stns177LonNeg[j]
s177PacificIslandsNames <- stns177$sname[j]

s177PacificIslandsMean <- array(NA,dim=c(56,1))
s177PacificIslandsSD <- array(NA,dim=c(56,1))
s177PacificIslandsSE <- array(NA,dim=c(56,1))
for ( i in 1:55){
...
s177PacificIslandsMean[56] <- mean(s177PacificIslandsS, na.rm=TRUE)
s177PacificIslandsSD[56] <- sd(s177PacificIslandsS, na.rm=TRUE)
s177PacificIslandsSE[56] <- s177PacificIslandsSD[56]/
...
s177EPacificS <- s177PgrCorrectedRates[56,j]
s177EPacific <- s177PgrCorrectedRates[1:55,j]
s177EPacificLat <- stns177Lat[j]
s177EPacificLon <- stns177LonNeg[j]
s177EPacificNames <- stns177$sname[j]

s177EPacificMean <- array(NA,dim=c(56,1))
s177EPacificSD <- array(NA,dim=c(56,1))
s177EPacificSE <- array(NA,dim=c(56,1))
for ( i in 1:55){
...
s177EPacificMean[56] <- mean(s177EPacificS, na.rm=TRUE)
s177EPacificSD[56] <- sd(s177EPacificS, na.rm=TRUE)
s177EPacificSE[56] <- s177EPacificSD[56]/
...
s177SEPacificS <- s177PgrCorrectedRates[56,j]
s177SEPacific <- s177PgrCorrectedRates[1:55,j]
s177SEPacificLat <- stns177Lat[j]
s177SEPacificLon <- stns177LonNeg[j]
s177SEPacificNames <- stns177$sname[j]

s177SEPacificMean <- array(NA,dim=c(56,1))
s177SEPacificSD <- array(NA,dim=c(56,1))
s177SEPacificSE <- array(NA,dim=c(56,1))
for ( i in 1:55){
...
s177SEPacificMean[56] <- mean(s177SEPacificS, na.rm=TRUE)
s177SEPacificSD[56] <- sd(s177SEPacificS, na.rm=TRUE)
s177SEPacificSE[56] <- s177SEPacificSD[56]/
...
s177SWAtlanticS <- s177PgrCorrectedRates[56,j]
s177SWAtlantic <- s177PgrCorrectedRates[1:55,j]
s177SWAtlanticLat <- stns177Lat[j]
s177SWAtlanticLon <- stns177LonNeg[j]
s177SWAtlanticNames <- stns177$sname[j]

s177SWAtlanticMean <- array(NA,dim=c(56,1))
s177SWAtlanticSD <- array(NA,dim=c(56,1))
s177SWAtlanticSE <- array(NA,dim=c(56,1))
for ( i in 1:55){
...
s177SWAtlanticMean[56] <- mean(s177SWAtlanticS, na.rm=TRUE)
s177SWAtlanticSD[56] <- sd(s177SWAtlanticS, na.rm=TRUE)
s177SWAtlanticSE[56] <- s177SWAtlanticSD[56]/
...
s177USGulfS <- s177PgrCorrectedRates[56,j]
s177USGulf <- s177PgrCorrectedRates[1:55,j]
s177USGulfLat <- stns177Lat[j]
s177USGulfLon <- stns177LonNeg[j]
s177USGulfNames <- stns177$sname[j]

s177USGulfMean <- array(NA,dim=c(56,1))
s177USGulfSD <- array(NA,dim=c(56,1))
s177USGulfSE <- array(NA,dim=c(56,1))
for ( i in 1:55){
...
s177USGulfMean[56] <- mean(s177USGulfS, na.rm=TRUE)
s177USGulfSD[56] <- sd(s177USGulfS, na.rm=TRUE)
s177USGulfSE[56] <- s177USGulfSD[56]/sqrt(length(which(is.finite(s177USGulfS))))
...
s177WAtlanticS <- s177PgrCorrectedRates[56,j]
s177WAtlantic <- s177PgrCorrectedRates[1:55,j]
s177WAtlanticLat <- stns177Lat[j]
s177WAtlanticLon <- stns177LonNeg[j]
s177WAtlanticNames <- stns177$sname[j]

s177WAtlanticMean <- array(NA,dim=c(56,1))
s177WAtlanticSD <- array(NA,dim=c(56,1))
s177WAtlanticSE <- array(NA,dim=c(56,1))
for ( i in 1:55){
...
s177WAtlanticMean[56] <- mean(s177WAtlanticS, na.rm=TRUE)
s177WAtlanticSD[56] <- sd(s177WAtlanticS, na.rm=TRUE)
s177WAtlanticSE[56] <- s177WAtlanticSD[56]/
...
s177NWAtlanticS <- s177PgrCorrectedRates[56,j]
s177NWAtlantic <- s177PgrCorrectedRates[1:55,j]
s177NWAtlanticLat <- stns177Lat[j]
s177NWAtlanticLon <- stns177LonNeg[j]
s177NWAtlanticNames <- stns177$sname[j]

s177NWAtlanticMean <- array(NA,dim=c(56,1))
s177NWAtlanticSD <- array(NA,dim=c(56,1))
s177NWAtlanticSE <- array(NA,dim=c(56,1))
for ( i in 1:55){
...
s177NWAtlanticMean[56] <- mean(s177NWAtlanticS, na.rm=TRUE)
s177NWAtlanticSD[56] <- sd(s177NWAtlanticS, na.rm=TRUE)
s177NWAtlanticSE[56] <- s177NWAtlanticSD[56]/
...
s177GlobalArray <- array(c(s177ScandinaviaMean, s177NEuropeMean, 
  ...
  s177NWAtlanticMean),dim=c(56,13))
s177GlobalMean <- array(NA,dim=c(56,1))
s177GlobalSD <- array(NA,dim=c(56,1))
s177GlobalSE <- array(NA,dim=c(56,1))
