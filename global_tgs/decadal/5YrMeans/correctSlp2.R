...
s177SlpAnnualYrs<-c(1850:2007)
s177SlpRates <- array(dim = c(lenRatesP1,177))
s177SlpDecadeMidPoints <- c(startRate:endRateP1)
for (i in 1:177){
  for (j in startYr:lastRate) {
    interval <- findInterval(j:(j+rateLenM1),s177SlpAnnualYrs)
    fit <- lsfit(s177SlpAnnualYrs[interval], ssh177Stns[interval, i])
    coeffs <- coef(fit)
    s177SlpRates[(j-startYrM1),i] <- coeffs[2]
  }
}
