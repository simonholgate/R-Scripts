...
load("pgrRates.RData")
#
s177PgrCorrectedRates <- array(NA,dim = c(56,177))
...
s177PgrCorrMean <- array(NA,dim = c(56))
s177PgrCorrSD <- array(NA,dim = c(56))
s177PgrCorrSE <- array(NA,dim = c(56))
for (i in 1:56){
...
#
# Demean stations
demeaned177Stations <- array(NA,dim = c(60,177))
for (i in 1:177){
  demeaned177Stations[,i] <- 
    stns177Annual[,i] - mean(stns177Annual[,i],na.rm=TRUE)
}
#
stns177Mean <- array(NA,dim = c(59))
stns177SD <- array(NA,dim = c(59))
stns177SE <- array(NA,dim = c(59))
for (i in 1:60){
...
par(lab=c(9,10,7))
plot(s177DecadeMidPoints[1:56], s177PgrCorrMean, type="l",ann=FALSE, lwd=2, 
  ylim=c(-14,14), yaxs="i",xaxs="i")
lines(s177DecadeMidPoints[1:56], s177PgrCorrMean+s177PgrCorrSE, lty="dashed")
lines(s177DecadeMidPoints[1:56], s177PgrCorrMean-s177PgrCorrSE, lty="dashed")
...
plot(s177DecadeMidPoints[1:56], 
  cumsum(s177PgrCorrMean), type="l",ann=FALSE, lwd=2)
lines(s177DecadeMidPoints[1:56], 
...
#quartz(height=6,width=6)
x11(height=6,width=6)
plot(s177DecadeMidPoints[1:56], cumsum(s177PgrCorrMean),ann=FALSE,
lty="twodash", type="l", xlim=c(1948,2006),ylim=c(-40,25),xaxs='i', lwd=2)
grid(col="black")
