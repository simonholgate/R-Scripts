...
s177ScandinaviaS <- s177PgrCorrectedRates[44,j]
s177Scandinavia <- s177PgrCorrectedRates[1:43,j]
s177ScandinaviaLat <- stns177Lat[j]
s177ScandinaviaLon <- stns177LonNeg[j]

s177ScandinaviaMean <- array(NA,dim=c(44,1))
s177ScandinaviaSD <- array(NA,dim=c(44,1))
s177ScandinaviaSE <- array(NA,dim=c(44,1))
for ( i in 1:43){
...
s177ScandinaviaMean[44] <- mean(s177ScandinaviaS, na.rm=TRUE)
s177ScandinaviaSD[44] <- sd(s177ScandinaviaS, na.rm=TRUE)
s177ScandinaviaSE[44] <- s177ScandinaviaSD[44]/
...
s177NEuropeS <- s177PgrCorrectedRates[44,j]
s177NEurope <- s177PgrCorrectedRates[1:43,j]
s177NEuropeLat <- stns177Lat[j]
s177NEuropeLon <- stns177LonNeg[j]

s177NEuropeMean <- array(NA,dim=c(44,1))
s177NEuropeSD <- array(NA,dim=c(44,1))
s177NEuropeSE <- array(NA,dim=c(44,1))
for ( i in 1:43){
...
s177NEuropeMean[44] <- mean(s177NEuropeS, na.rm=TRUE)
s177NEuropeSD[44] <- sd(s177NEuropeS, na.rm=TRUE)
s177NEuropeSE[44] <- s177NEuropeSD[44]/
...
s177EAtlanticS <- s177PgrCorrectedRates[44,j]
s177EAtlantic <- s177PgrCorrectedRates[1:43,j]
s177EAtlanticLat <- stns177Lat[j]
s177EAtlanticLon <- stns177LonNeg[j]

s177EAtlanticMean <- array(NA,dim=c(44,1))
s177EAtlanticSD <- array(NA,dim=c(44,1))
s177EAtlanticSE <- array(NA,dim=c(44,1))
for ( i in 1:43){
...
s177EAtlanticMean[44] <- mean(s177EAtlanticS, na.rm=TRUE)
s177EAtlanticSD[44] <- sd(s177EAtlanticS, na.rm=TRUE)
s177EAtlanticSE[44] <- s177EAtlanticSD[44]/
...
s177MediterraneanS <- s177PgrCorrectedRates[44,j]
s177Mediterranean <- s177PgrCorrectedRates[1:43,j]
s177MediterraneanLat <- stns177Lat[j]
s177MediterraneanLon <- stns177LonNeg[j]

s177MediterraneanMean <- array(NA,dim=c(44,1))
s177MediterraneanSD <- array(NA,dim=c(44,1))
s177MediterraneanSE <- array(NA,dim=c(44,1))
for ( i in 1:43){
...
s177MediterraneanMean[44] <- mean(s177MediterraneanS, na.rm=TRUE)
s177MediterraneanSD[44] <- sd(s177MediterraneanS, na.rm=TRUE)
s177MediterraneanSE[44] <- s177MediterraneanSD[44]/
...
s177SEAsiaS <- s177PgrCorrectedRates[44,j]
s177SEAsia <- s177PgrCorrectedRates[1:43,j]
s177SEAsiaLat <- stns177Lat[j]
s177SEAsiaLon <- stns177LonNeg[j]

s177SEAsiaMean <- array(NA,dim=c(44,1))
s177SEAsiaSD <- array(NA,dim=c(44,1))
s177SEAsiaSE <- array(NA,dim=c(44,1))
for ( i in 1:43){
...
s177SEAsiaMean[44] <- mean(s177SEAsiaS, na.rm=TRUE)
s177SEAsiaSD[44] <- sd(s177SEAsiaS, na.rm=TRUE)
s177SEAsiaSE[44] <- s177SEAsiaSD[44]/sqrt(length(which(is.finite(s177SEAsiaS))))
...
s177AustralasiaS <- s177PgrCorrectedRates[44,j]
s177Australasia <- s177PgrCorrectedRates[1:43,j]
s177AustralasiaLat <- stns177Lat[j]
s177AustralasiaLon <- stns177LonNeg[j]

s177AustralasiaMean <- array(NA,dim=c(44,1))
s177AustralasiaSD <- array(NA,dim=c(44,1))
s177AustralasiaSE <- array(NA,dim=c(44,1))
for ( i in 1:43){
...
s177AustralasiaMean[44] <- mean(s177AustralasiaS, na.rm=TRUE)
s177AustralasiaSD[44] <- sd(s177AustralasiaS, na.rm=TRUE)
s177AustralasiaSE[44] <- s177AustralasiaSD[44]/
...
s177PacificIslandsS <- s177PgrCorrectedRates[44,j]
s177PacificIslands <- s177PgrCorrectedRates[1:43,j]
s177PacificIslandsLat <- stns177Lat[j]
s177PacificIslandsLon <- stns177LonNeg[j]

s177PacificIslandsMean <- array(NA,dim=c(44,1))
s177PacificIslandsSD <- array(NA,dim=c(44,1))
s177PacificIslandsSE <- array(NA,dim=c(44,1))
for ( i in 1:43){
...
s177PacificIslandsMean[44] <- mean(s177PacificIslandsS, na.rm=TRUE)
s177PacificIslandsSD[44] <- sd(s177PacificIslandsS, na.rm=TRUE)
s177PacificIslandsSE[44] <- s177PacificIslandsSD[44]/
...
s177EPacificS <- s177PgrCorrectedRates[44,j]
s177EPacific <- s177PgrCorrectedRates[1:43,j]
s177EPacificLat <- stns177Lat[j]
s177EPacificLon <- stns177LonNeg[j]

s177EPacificMean <- array(NA,dim=c(44,1))
s177EPacificSD <- array(NA,dim=c(44,1))
s177EPacificSE <- array(NA,dim=c(44,1))
for ( i in 1:43){
...
s177EPacificMean[44] <- mean(s177EPacificS, na.rm=TRUE)
s177EPacificSD[44] <- sd(s177EPacificS, na.rm=TRUE)
s177EPacificSE[44] <- s177EPacificSD[44]/
...
s177SEPacificS <- s177PgrCorrectedRates[44,j]
s177SEPacific <- s177PgrCorrectedRates[1:43,j]
s177SEPacificLat <- stns177Lat[j]
s177SEPacificLon <- stns177LonNeg[j]

s177SEPacificMean <- array(NA,dim=c(44,1))
s177SEPacificSD <- array(NA,dim=c(44,1))
s177SEPacificSE <- array(NA,dim=c(44,1))
for ( i in 1:43){
...
s177SEPacificMean[44] <- mean(s177SEPacificS, na.rm=TRUE)
s177SEPacificSD[44] <- sd(s177SEPacificS, na.rm=TRUE)
s177SEPacificSE[44] <- s177SEPacificSD[44]/
...
s177SWAtlanticS <- s177PgrCorrectedRates[44,j]
s177SWAtlantic <- s177PgrCorrectedRates[1:43,j]
s177SWAtlanticLat <- stns177Lat[j]
s177SWAtlanticLon <- stns177LonNeg[j]

s177SWAtlanticMean <- array(NA,dim=c(44,1))
s177SWAtlanticSD <- array(NA,dim=c(44,1))
s177SWAtlanticSE <- array(NA,dim=c(44,1))
for ( i in 1:43){
...
s177SWAtlanticMean[44] <- mean(s177SWAtlanticS, na.rm=TRUE)
s177SWAtlanticSD[44] <- sd(s177SWAtlanticS, na.rm=TRUE)
s177SWAtlanticSE[44] <- s177SWAtlanticSD[44]/
...
s177USGulfS <- s177PgrCorrectedRates[44,j]
s177USGulf <- s177PgrCorrectedRates[1:43,j]
s177USGulfLat <- stns177Lat[j]
s177USGulfLon <- stns177LonNeg[j]

s177USGulfMean <- array(NA,dim=c(44,1))
s177USGulfSD <- array(NA,dim=c(44,1))
s177USGulfSE <- array(NA,dim=c(44,1))
for ( i in 1:43){
...
s177USGulfMean[44] <- mean(s177USGulfS, na.rm=TRUE)
s177USGulfSD[44] <- sd(s177USGulfS, na.rm=TRUE)
s177USGulfSE[44] <- s177USGulfSD[44]/sqrt(length(which(is.finite(s177USGulfS))))
...
s177WAtlanticS <- s177PgrCorrectedRates[44,j]
s177WAtlantic <- s177PgrCorrectedRates[1:43,j]
s177WAtlanticLat <- stns177Lat[j]
s177WAtlanticLon <- stns177LonNeg[j]

s177WAtlanticMean <- array(NA,dim=c(44,1))
s177WAtlanticSD <- array(NA,dim=c(44,1))
s177WAtlanticSE <- array(NA,dim=c(44,1))
for ( i in 1:43){
...
s177WAtlanticMean[44] <- mean(s177WAtlanticS, na.rm=TRUE)
s177WAtlanticSD[44] <- sd(s177WAtlanticS, na.rm=TRUE)
s177WAtlanticSE[44] <- s177WAtlanticSD[44]/
...
s177NWAtlanticS <- s177PgrCorrectedRates[44,j]
s177NWAtlantic <- s177PgrCorrectedRates[1:43,j]
s177NWAtlanticLat <- stns177Lat[j]
s177NWAtlanticLon <- stns177LonNeg[j]

s177NWAtlanticMean <- array(NA,dim=c(44,1))
s177NWAtlanticSD <- array(NA,dim=c(44,1))
s177NWAtlanticSE <- array(NA,dim=c(44,1))
for ( i in 1:43){
...
s177NWAtlanticMean[44] <- mean(s177NWAtlanticS, na.rm=TRUE)
s177NWAtlanticSD[44] <- sd(s177NWAtlanticS, na.rm=TRUE)
s177NWAtlanticSE[44] <- s177NWAtlanticSD[44]/
...
s177GlobalArray <- array(c(s177ScandinaviaMean, s177NEuropeMean, 
  ...
  s177NWAtlanticMean),dim=c(44,13))
s177GlobalMean <- array(NA,dim=c(44,1))
s177GlobalSD <- array(NA,dim=c(44,1))
s177GlobalSE <- array(NA,dim=c(44,1))
