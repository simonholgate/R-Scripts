...
load("pgrRates.RData")
#
s177PgrCorrectedRates <- array(NA,dim = c(46,177))
...
s177PgrCorrMean <- array(NA,dim = c(46))
s177PgrCorrSD <- array(NA,dim = c(46))
s177PgrCorrSE <- array(NA,dim = c(46))
for (i in 1:46){
...
#
# Demean stations
demeaned177Stations <- array(NA,dim = c(55,177))
for (i in 1:177){
  demeaned177Stations[,i] <- 
    stns177Annual[,i] - mean(stns177Annual[,i],na.rm=TRUE)
}
#
stns177Mean <- array(NA,dim = c(55))
stns177SD <- array(NA,dim = c(55))
stns177SE <- array(NA,dim = c(55))
for (i in 1:55){
...
par(lab=c(9,10,7))
plot(s177DecadeMidPoints[1:46], s177PgrCorrMean, type="l",ann=FALSE, lwd=2, 
  ylim=c(-8,14), yaxs="i",xaxs="i")
lines(s177DecadeMidPoints[1:46], s177PgrCorrMean+s177PgrCorrSE, lty="dashed")
lines(s177DecadeMidPoints[1:46], s177PgrCorrMean-s177PgrCorrSE, lty="dashed")
...
plot(s177DecadeMidPoints[1:46], 
  cumsum(s177PgrCorrMean), type="l",ann=FALSE, lwd=2)
lines(s177DecadeMidPoints[1:46], 
...
#quartz(height=6,width=6)
x11(height=6,width=6)
plot(s177DecadeMidPoints[1:46], cumsum(s177PgrCorrMean),ann=FALSE,
lty="twodash", type="l", xlim=c(1948,1998),ylim=c(-20,240),xaxs='i')
grid(col="black")
