...
# This version uses monthly data in order to reduce the number of gaps which
# arise due to annual values having not been calculated.
#
...
rateLenM1 <- rateLen-1
startYr<-1992
# Each monthly mean is centred on the month in which it was made, so the year
# starts 1/24th of the way into the year and ends at 23/24ths
startMon<-1992+(1/24)
startYrM1<-startYr-1
startMonM1<-startMon-1
endYr<-2008
endMon<-2008+(23/24)
...
stnsMonthlyYrs<-seq(from=startMon,to=endMon,by=1/12)

startRate<-startMon+(rateLen*1/12)/2
endRate<-endMon-(rateLen*1/12)/2
endRateP1<-endRate+1/12
lastRate<-endMon-(rateLenM1*1/12)
lenRates<-length(seq(from=startRate, to=endRate, by=1/12))
...
#source("getStationsMonthly.R")
load("altimeterPeriodStnsMonthly.RData")

# Calculate the rates 
source("~/workspace/RScripts/global_tgs/pentadal/AltimetryPeriod/monthly/longRecsMonthly.R")

# Correct for pressure changes
source("~/workspace/RScripts/global_tgs/pentadal/AltimetryPeriod/monthly/correctSlp.R")
source("~/workspace/RScripts/global_tgs/pentadal/AltimetryPeriod/monthly/pressureCorrectedRates.R")

# PGR correct these rates
source("~/workspace/RScripts/global_tgs/pentadal/AltimetryPeriod/monthly/pgrCorrectRates.R")

# Group into the 13 regions of the original study and calculate a global mean
source("~/workspace/RScripts/global_tgs/pentadal/AltimetryPeriod/monthly/group13Regions.R")
