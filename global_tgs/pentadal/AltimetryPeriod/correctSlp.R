...
con<-file("~/diskx/HadSLP2r/hadSLP2r.ann.bin","rb")
...
slpAnnualYrs<-c(1850:2007)
slpRates <- array(dim = c(lenRatesP1,numStns))
slpDecadeMidPoints <- c(startRate:endRateP1)
for (i in 1:numStns){
...
# Save the data
save(slpDecadeMidPoints,slpRates,file="stnsSlpRates.RData")
