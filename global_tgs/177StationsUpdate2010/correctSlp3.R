...
con<-file("../HadSLP2r/hadSLP2r.ann.bin","rb")
...
s177SlpAnnualYrs<-c(1850:2009)
# Add last point for altimetry period 1992-2009
s177SlpRates <- array(dim = c(59,177))
s177SlpDecadeMidPoints <- array(dim = c(58,1))
for (i in 1:177){
  for (j in 1948:2000) {
    interval <- findInterval(j:(j+9),s177SlpAnnualYrs)
    data <- data.frame(Year=s177SlpAnnualYrs[interval], SLP=ssh177Stns[interval, i])
    fit <- lmRob(SLP ~ Year, data=data)
    s177SlpRates[(j-1947),i] <- coef(fit)[2]
    s177SlpDecadeMidPoints[j-1947] <- j+4
  }
  ####################
  # Altimetry period #
  ####################
    interval <- findInterval(1992:2009,s177SlpAnnualYrs)
    data <- data.frame(Year=s177SlpAnnualYrs[interval], SLP=ssh177Stns[interval, i])
    fit <- lmRob(SLP ~ Year, data=data)
    s177SlpRates[59,i] <- coef(fit)[2]
}
