...
tg$mp.resid <- tg$dm.mp.brest - tg$dm.mp.newlyn

tg$mp.brest.common <- tg$dm.mp.brest - tg$mp.resid
tg$mp.newlyn.common <- tg$dm.mp.newlyn - tg$mp.resid


tg$dm.brest <- tg$brestMonthly-mean(tg$brestMonthly, na.rm=T)
tg$dm.newlyn <- tg$newlynMonthly-mean(tg$newlynMonthly, na.rm=T)
tg$dm.resid <- tg$dm.brest-tg$dm.newlyn

tg$dm.mean <- (tg$dm.brest + tg$dm.newlyn)/2

# Calculate annual means
tg$numYrs <- length(tg$annualYrs)

tg$annualYrs <- seq.Date(from=as.Date("1914/7/1"), to=as.Date("2006/7/1"), by="1 year")

tg$tmp <- tg$dm.brest
dim(tg$tmp) <- c(12,tg$numYrs)
tg$annualMeans.brest <- colMeans(tg$tmp)

tg$tmp <- tg$dm.newlyn
dim(tg$tmp) <- c(12,tg$numYrs)
tg$annualMeans.newlyn <- colMeans(tg$tmp)

tg$tmp <- tg$dm.resid
dim(tg$tmp) <- c(12,tg$numYrs)
tg$annualMeans.resid <- colMeans(tg$tmp)

tg$annualMeans.mean <- (tg$annualMeans.brest + tg$annualMeans.newlyn)/2

rm(tmp, envir=tg)

# Produce decadal means to examine reduction in decadal variance
tg$tenYrTrends.brest <- vector(mode='numeric',length=(tg$numYrs-9))
tg$resid.tenYrTrends.brest <- tg$tenYrTrends.brest 
tg$fitted.tenYrTrends.brest <- tg$tenYrTrends.brest
tg$fitted.tenYrMeans.brest <- tg$tenYrTrends.brest

tg$tenYrTrends.newlyn <- vector(mode='numeric',length=(tg$numYrs-9))
tg$resid.tenYrTrends.newlyn <- tg$tenYrTrends.newlyn 
tg$fitted.tenYrTrends.newlyn <- tg$tenYrTrends.newlyn
tg$tenYrMeans.newlyn <- tg$tenYrTrends.newlyn

tg$midpointYrs <- seq.Date(from=as.Date("1919/1/1"), length=(tg$numYrs-9), by="1 year")
for (i in 1:(tg$numYrs-9)) {
       
     	if (length(which(is.finite(tg$annualMeans.brest[i:(i+9)])))>=7){
                tg$tenYrMeans.brest[i] <- mean(tg$annualMeans.brest[i:(i+9)], na.rm=T)
		tg$junk <- lm(tg$annualMeans.brest[i:(i+9)] ~ c(1:10), na.action = na.exclude)
		tg$tenYrTrends.brest[i] <- tg$junk$coef[2]
	} else {
		tg$tenYrTrends.brest[i] <- NA
                tg$tenYrMeans.brest[i] <- NA
	}
	if (length(which(is.finite(tg$fitted.annualMeans.brest[i:(i+9)])))>=7){
		tg$junk <- lm(tg$fitted.annualMeans.brest[i:(i+9)] ~ c(1:10), na.action = na.exclude)
		tg$fitted.tenYrTrends.brest[i] <- tg$junk$coef[2]
	} else {
		tg$fitted.tenYrTrends.brest[i] <- NA
	}
	if (length(which(is.finite(tg$resid.annualMeans.newlyn[i:(i+9)])))>=7){
		tg$junk <- lm(tg$resid.annualMeans.newlyn[i:(i+9)] ~ c(1:10), na.action = na.exclude)
		tg$resid.tenYrTrends.newlyn[i] <- tg$junk$coef[2]
	} else {
		tg$resid.tenYrTrends.newlyn[i] <- NA
	}
	if (length(which(is.finite(tg$annualMeans.newlyn[i:(i+9)])))>=7){
		tg$junk <- lm(tg$annualMeans.newlyn[i:(i+9)] ~ c(1:10), na.action = na.exclude)
		tg$tenYrTrends.newlyn[i] <- tg$junk$coef[2]
                tg$tenYrMeans.newlyn[i] <- mean(tg$annualMeans.newlyn[i:(i+9)], na.rm=T)
	} else {
		tg$tenYrTrends.newlyn[i] <- NA
                tg$tenYrMeans.newlyn[i] <- NA
	}
	if (length(which(is.finite(tg$fitted.annualMeans.newlyn[i:(i+9)])))>=7){
		tg$junk <- lm(tg$fitted.annualMeans.newlyn[i:(i+9)] ~ c(1:10), na.action = na.exclude)
		tg$fitted.tenYrTrends.newlyn[i] <- tg$junk$coef[2]
	} else {
		tg$fitted.tenYrTrends.newlyn[i] <- NA
	}
	if (length(which(is.finite(tg$resid.annualMeans.newlyn[i:(i+9)])))>=7){
		tg$junk <- lm(tg$resid.annualMeans.newlyn[i:(i+9)] ~ c(1:10), na.action = na.exclude)
		tg$resid.tenYrTrends.newlyn[i] <- tg$junk$coef[2]
...
tg$tenYrTrends.resid <- tg$tenYrTrends.brest - tg$tenYrTrends.newlyn
tg$tenYrTrends.mean <- (tg$tenYrTrends.brest + tg$tenYrTrends.newlyn)/2
tg$tenYrMeans.resid <- tg$tenYrMeans.brest - tg$tenYrMeans.newlyn
tg$tenYrMeans.mean <- (tg$tenYrMeans.brest + tg$tenYrMeans.newlyn)/2


# The residual from simply subtracting the model from the TG
...
# Spectra etc
tg$ts.newlyn<-ts(data=tg$dm.newlyn, start=c(1914,1), frequency=12)
tg$ts.brest<-ts(data=tg$dm.brest, start=c(1914,1), frequency=12)
plot(tg$ts.brest)
tg$spc <- spec.pgram(ts.union(tg$ts.brest,tg$ts.newlyn), kernel=kernel("daniell", c(11,7,3)))
...
plymouth$totp.tg.mp.brest <- 1025*9.8*tg$mp.brest/1000
# Plymouth pressure ends in Dec 2004
plymouth$totp.tg.brest <- 1025*9.8*tg$brestMonthly[1:1092]/1000
plymouth$totp.m.brest <- 1025*9.8*S12R408$brestMonthlyMean[1:480,1]
#

plymouth$totp.tg.mp.brest <- plymouth$totp.tg.mp.brest + plymouth$Pm*100
plymouth$totp.tg.brest <- plymouth$totp.tg.brest + plymouth$Ptg*100
plymouth$totp.m.brest <- plymouth$totp.m.brest + plymouth$Pm*100

plymouth$totp.dm.tg.mp.brest <- plymouth$totp.tg.mp.brest - mean(plymouth$totp.tg.mp.brest, na.rm=T)
plymouth$totp.dm.tg.brest <- plymouth$totp.tg.brest - mean(plymouth$totp.tg.brest, na.rm=T)
plymouth$totp.dm.m.brest <- plymouth$totp.m.brest - mean(plymouth$totp.m.brest, na.rm=T)
...
# Hadley SLP is in mb so need to convert to Pa by multiplying by 100
hadley$totp.tg.mp.brest <- hadley$totp.tg.mp.brest + hadley$mp.slp[,1]*100
hadley$totp.tg.brest <- hadley$totp.tg.brest + hadley$n.slp[,1]*100
hadley$totp.m.brest <- hadley$totp.m.brest + hadley$mp.slp[,1]*100

hadley$totp.dm.tg.mp.brest <- hadley$totp.tg.mp.brest - mean(hadley$totp.tg.mp.brest, na.rm=T)
hadley$totp.dm.tg.brest <- hadley$totp.tg.brest - mean(hadley$totp.tg.brest, na.rm=T)
hadley$totp.dm.m.brest <- hadley$totp.m.brest - mean(hadley$totp.m.brest, na.rm=T)
...
# Zero lag
for(i in 1:9){
	model4c$Pm[,i] <- as.vector(hadley$mp.slp[,(i+7)])*100
}
# Lag 1
for(i in 10:18){
	model4c$Pm[2:480,i] <- as.vector(hadley$mp.slp[,(i-2)])[1:479]*100
	model4c$Pm[1,i] <- mean(as.vector(hadley$mp.slp[,(i-2)])[1:479], na.rm=T)*100
}
# Lag 2
for(i in 19:27){
	model4c$Pm[3:480,i] <- as.vector(hadley$mp.slp[,(i-11)])[1:478]*100
	model4c$Pm[1:2,i] <- mean(as.vector(hadley$mp.slp[,(i-11)])[1:478], na.rm=T)*100
}
...
#[1] 0.07098115 Newlyn: [1] 0.0676091 (model4)
#[1] 0.05650118 Newlyn: [1] 0.07649279 (model4c)

#> sd(model4c$m.resid.brest/(1025*9.8), na.rm=T)
#[1] 0.02322259 Newlyn: [1] 0.02422649 (model4)
#[1] 0.03218805 Newlyn: [1] 0.06616694 (model4c)

#> sd(model4c$data.brest$msl/(1025*9.8), na.rm=T)
#[1] 0.06249704 Newlyn: 82.10498
#> sd(model4c$tg.resid.brest/(1025*9.8), na.rm=T)
#[1] 0.04128576 Newlyn: 25.16201
# No signficant improvement
...
for(i in 1:18){
	model5c$data.brest <- data.frame(mmsl=hadley$totp.m.brest, msl=hadley$totp.tg.mp.brest,
...
x11()
plot(tg$mp.time,hadley$totp.dm.m.brest, type='l', col='blue')
lines(tg$mp.time, (model5c$m.fitted.brest-mean(model5c$m.fitted.brest, na.rm=T)), col='red')
lines(tg$mp.time, model5c$m.resid.brest, col='magenta', lwd=2)
...
x11()
plot(tg$mp.time,hadley$totp.dm.tg.mp.brest, type='l', col='blue')
...
#> sd(model5c$data.brest$mmsl/(1025*9.8), na.rm=T)
#[1] 0.07098115 Newlyn: 0.0676091 (model5)
#[1] 0.07945347 Newlyn: 0.07649279 (model5c)
#> sd(model5c$m.resid.brest/(1025*9.8), na.rm=T)
#[1] 0.01703225 Newlyn: 0.01615011 (model5)
#[1] 0.02085466 Newlyn: 0.2276618 (model5c)

#> sd(model5c$data.brest$msl/(1025*9.8), na.rm=T)
#[1] 0.0866822 Newlyn: 82.10498
#> sd(model5c$tg.resid.brest/(1025*9.8), na.rm=T)
#[1] 30.08910 Newlyn: 23.78966 (model5)
#[1] 0.0403727 Newlyn: 23.35431 (model5c)
...
model5c$tmp <- hadley$totp.dm.tg.mp.brest
...
#[1] 1921.118 Newlyn: 15.47344
#var(model5c$tg.resid.tenYrTrends.brest, na.rm=T)
#[1] 8.113846 Newlyn: 5.570029 (model5)
#[1] 2053.433 Newlyn: 3.963082 (model5c)
...
## Model 4d ##
##############
# As with model 4b (using S12R408) but using Plymouth data.
# Also, put local pressure term on the LHS
# to avoid aliasing and use total pressure.
# S12R408 was run for longer, so we need to truncate it at 480 months
# 
...
# Zero lag
for(i in 1:9){
	model4d$Pm[,i] <- as.vector(hadley$mp.slp[,(i+7)])*100
}
# Lag 1
for(i in 10:18){
	model4d$Pm[2:480,i] <- as.vector(hadley$mp.slp[,(i-2)])[1:479]*100
	model4d$Pm[1,i] <- mean(as.vector(hadley$mp.slp[,(i-2)])[1:479], na.rm=T)*100
}
# Lag 2
for(i in 19:27){
	model4d$Pm[3:480,i] <- as.vector(hadley$mp.slp[,(i-11)])[1:478]*100
	model4d$Pm[1:2,i] <- mean(as.vector(hadley$mp.slp[,(i-11)])[1:478], na.rm=T)*100
}
...
model4d$t <- seq(from=0,to=((40*12)-1))

for(i in 1:27){
	model4d$data.brest <- data.frame(mmsl=plymouth$totp.m.brest, msl=plymouth$totp.tg.mp.brest,
			t=model4d$t, w1cos=cos(w1*model4d$t), w1sin=sin(w1*model4d$t), 
		        w2cos=cos(w2*model4d$t), w2sin=sin(w2*model4d$t), Pm=model4d$Pm[,i])
	
	model4d$m.lm.brest <- lm(mmsl ~ t + w1cos + w1sin + w2cos + w2sin + Pm, data=model4d$data.brest)
	model4d$tg.lm.brest <- lm(msl ~ t + w1cos + w1sin + w2cos + w2sin + Pm, data=model4d$data.brest)
...
model4d$data.brest <- data.frame(mmsl=plymouth$totp.m.brest, msl=plymouth$totp.tg.mp.brest,
			t=model4d$t, w1cos=cos(w1*model4d$t), w1sin=sin(w1*model4d$t), 
		        w2cos=cos(w2*model4d$t), w2sin=sin(w2*model4d$t), Pm1=model4d$Pm[,model4d$m.sd.sorted$ix[1]], 
...
model4d$m.lm.brest <- lm(mmsl ~ t + w1cos + w1sin + w2cos + w2sin + 
				Pm1 + Pm2 + Pm3 + Pm4 + Pm5 + Pm6 + Pm7 + Pm8 + Pm9,  data=model4d$data.brest)

model4d$data.brest <- data.frame(mmsl=plymouth$totp.m.brest, msl=plymouth$totp.tg.mp.brest,
			t=model4d$t, w1cos=cos(w1*model4d$t), w1sin=sin(w1*model4d$t), 
		        w2cos=cos(w2*model4d$t), w2sin=sin(w2*model4d$t), Pm1=model4d$Pm[,model4d$tg.sd.sorted$ix[1]], 
...
model4d$tg.lm.brest <- lm(msl ~ t + w1cos + w1sin + w2cos + w2sin + 
				Pm1 + Pm2 + Pm3 + Pm4 + Pm5 + Pm6 + Pm7 + Pm8 + Pm9,  data=model4d$data.brest)
...
#> sd(model4d$data.brest$mmsl/(1025*9.8), na.rm=T)
#[1] 0.07098115 Newlyn: [1] 0.0676091 (model4)
#[1] 0.07945347 Newlyn: [1] 0.07649279 (model4d)

#> sd(model4d$m.resid.brest/(1025*9.8), na.rm=T)
#[1] 0.02322259 Newlyn: [1] 0.02422649 (model4)
#[1] 0.03283935 Newlyn: [1] 0.06616694 (model4d)

#> sd(model4d$data.brest$msl/(1025*9.8), na.rm=T)
#[1] 0.0866822 Newlyn: 82.10498
#> sd(model4d$tg.resid.brest/(1025*9.8), na.rm=T)
#[1] 0.04351248 Newlyn: 25.16201
# No signficant improvement

###############
## Brest SLP ##
###############
# Try using the local Brest pressure (since 1951) rather than Plymouth
brestSLP <- new.env()
load("~/diskx/polcoms/brestNewlyn/pressure/brestPressures.RData", envir=brestSLP)


# Just use pressures over the periods of the model data (Pm) and TG data (Ptg)
brestSLP$Pm <- brestSLP$pressure[intersect(which(brestSLP$time>=as.Date("1960/1/15")), which(brestSLP$time<=as.Date("1999/12/15")))]
brestSLP$Ptg <- brestSLP$pressure[intersect(which(brestSLP$time>=as.Date("1914/1/15")), which(brestSLP$time<=as.Date("2006/12/15")))]

# We need to work in terms of total pressure rather than in sea level as in Thompson '86
brestSLP$totp.tg.mp.brest <- 1025*9.8*tg$mp.brest/1000
# BrestSLP pressure ends in Dec 2001
brestSLP$totp.tg.brest <- 1025*9.8*tg$brestMonthly/1000
brestSLP$totp.m.brest <- 1025*9.8*S12R408$brestMonthlyMean[1:480,1]
#

brestSLP$totp.tg.mp.brest <- brestSLP$totp.tg.mp.brest + brestSLP$Pm*100
brestSLP$totp.tg.brest <- brestSLP$totp.tg.brest + brestSLP$Ptg*100
brestSLP$totp.m.brest <- brestSLP$totp.m.brest + brestSLP$Pm*100

brestSLP$totp.dm.tg.mp.brest <- brestSLP$totp.tg.mp.brest - mean(brestSLP$totp.tg.mp.brest, na.rm=T)
brestSLP$totp.dm.tg.brest <- brestSLP$totp.tg.brest - mean(brestSLP$totp.tg.brest, na.rm=T)
brestSLP$totp.dm.m.brest <- brestSLP$totp.m.brest - mean(brestSLP$totp.m.brest, na.rm=T)
...
## Model 4e ##
##############
# As with model 4b (using S12R408) but using BrestSLP data.
# Also, put local pressure term on the LHS
# to avoid aliasing and use total pressure.
# S12R408 was run for longer, so we need to truncate it at 480 months
# 
...
# regression. We achieve this by adding in each component in turn and calculating
# the variance reduction, which we can then rank and analyze.
model4e$Pm <- array(NA,dim=c(480,27))
# Zero lag
for(i in 1:9){
	model4e$Pm[,i] <- as.vector(hadley$mp.slp[,(i+7)])*100
}
# Lag 1
for(i in 10:18){
	model4e$Pm[2:480,i] <- as.vector(hadley$mp.slp[,(i-2)])[1:479]*100
	model4e$Pm[1,i] <- mean(as.vector(hadley$mp.slp[,(i-2)])[1:479], na.rm=T)*100
}
# Lag 2
for(i in 19:27){
	model4e$Pm[3:480,i] <- as.vector(hadley$mp.slp[,(i-11)])[1:478]*100
	model4e$Pm[1:2,i] <- mean(as.vector(hadley$mp.slp[,(i-11)])[1:478], na.rm=T)*100
}

# What we are going to do now is calculate the sd of the residuals as each of the
# 27 components is added in turn 
model4e$m.sd <- vector(mode="numeric", length=27)
model4e$tg.sd <- vector(mode="numeric", length=27)
model4e$t <- seq(from=0,to=((40*12)-1))

for(i in 1:27){
	model4e$data.brest <- data.frame(mmsl=plymouth$totp.m.brest, msl=plymouth$totp.tg.mp.brest,
			t=model4e$t, w1cos=cos(w1*model4e$t), w1sin=sin(w1*model4e$t), 
		        w2cos=cos(w2*model4e$t), w2sin=sin(w2*model4e$t), Pm=model4e$Pm[,i])
	
	model4e$m.lm.brest <- lm(mmsl ~ t + w1cos + w1sin + w2cos + w2sin + Pm, data=model4e$data.brest)
	model4e$tg.lm.brest <- lm(msl ~ t + w1cos + w1sin + w2cos + w2sin + Pm, data=model4e$data.brest)
...
model4e$data.brest <- data.frame(mmsl=plymouth$totp.m.brest, msl=plymouth$totp.tg.mp.brest,
			t=model4e$t, w1cos=cos(w1*model4e$t), w1sin=sin(w1*model4e$t), 
		        w2cos=cos(w2*model4e$t), w2sin=sin(w2*model4e$t), Pm1=model4e$Pm[,model4e$m.sd.sorted$ix[1]], 
...
model4e$m.lm.brest <- lm(mmsl ~ t + w1cos + w1sin + w2cos + w2sin + 
				Pm1 + Pm2 + Pm3 + Pm4 + Pm5 + Pm6 + Pm7 + Pm8 + Pm9,  data=model4e$data.brest)

model4e$data.brest <- data.frame(mmsl=plymouth$totp.m.brest, msl=plymouth$totp.tg.mp.brest,
			t=model4e$t, w1cos=cos(w1*model4e$t), w1sin=sin(w1*model4e$t), 
		        w2cos=cos(w2*model4e$t), w2sin=sin(w2*model4e$t), Pm1=model4e$Pm[,model4e$tg.sd.sorted$ix[1]], 
...
model4e$tg.lm.brest <- lm(msl ~ t + w1cos + w1sin + w2cos + w2sin + 
				Pm1 + Pm2 + Pm3 + Pm4 + Pm5 + Pm6 + Pm7 + Pm8 + Pm9,  data=model4e$data.brest)
...
#> sd(model4e$data.brest$mmsl/(1025*9.8), na.rm=T)
#[1] 0.07098115 Newlyn: [1] 0.0676091 (model4)
#[1] 0.07945347 Newlyn: [1] 0.07649279 (model4e)

#> sd(model4e$m.resid.brest/(1025*9.8), na.rm=T)
#[1] 0.02322259 Newlyn: [1] 0.02422649 (model4)
#[1] 0.03283935 Newlyn: [1] 0.06616694 (model4e)

#> sd(model4e$data.brest$msl/(1025*9.8), na.rm=T)
#[1] 0.0866822 Newlyn: 82.10498
#> sd(model4e$tg.resid.brest/(1025*9.8), na.rm=T)
#[1] 0.04351248 Newlyn: 25.16201
# No signficant improvement
