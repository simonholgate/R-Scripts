...
tmp <- new.env()
# HadSLP2r data is 1850-2007 = 158 years = 1896 months. 32 cols - Newlyn, Brest and 16 grid points
load("~/Dropbox/brestNewlynData/pressure/brestNewlynHadSLP2Paper.RData", envir=tmp)
met$hadSLP2rAnnualPressureArray <- array(NA, dim=c(158,32))
for(i in 1:32){
  tmp$hadSLP2rMonthlyPressureArray <- tmp$slpNewlynStns[,i]
  dim(tmp$hadSLP2rMonthlyPressureArray) <- c(12,158)
  met$hadSLP2rAnnualPressureArray[,i] <- colMeans(tmp$hadSLP2rMonthlyPressureArray, na.rm=T)
}
met$hadSLP2rAnnualTime <- c(1850:2007)
rm(tmp)
...
model2$tg.lmRob.brestTotalP <- lmRob(model2$brestTotalP ~ model2$time)
model2$tg.lmRob.newlynTotalP <- lmRob(model2$newlynTotalP ~ model2$time)
...
# First two points are Newlyn and Brest
met$hadSLP2rUniq <- met$hadSLP2rUniq[3:30]

model2$Pa <- array(NA,dim=c(40,(28*3)))
# Zero lag
for(i in 1:28){
...
# Lag 1 year
for(i in 29:56){
  model2$Pa[,i] <- as.vector(met$hadSLP2rAnnualPressureArray[110:149,met$hadSLP2rUniq[(i-28)]])
}
# Lag 2 years....
for(i in 57:84){
  model2$Pa[,i] <- as.vector(met$hadSLP2rAnnualPressureArray[109:148,met$hadSLP2rUniq[(i-56)]])
}
...
# What we are going to do now is calculate the sd of the residuals as each of the
# 84 components is added in turn 
model2$tg.sd.brest <- vector(mode="numeric", length=84)

for(i in 1:84){
  model2$data.brest <- data.frame(msl=model2$brestTotalP, t=seq(from=1960,to=1999), Pa=model2$Pa[,i])
  model2$tg.lmRob.brest <- lmRob(msl ~ t + Pa, data=model2$data.brest)
...
# Variance reduction
#> (model2$tg.lmRob.brestTotalP$scale-model2$tg.lmRob.brest$scale)/model2$tg.lmRob.brestTotalP$scale
#[1] 0.3618851
#> model2$tg.lmRob.brest$scale/model2$tg.lmRob.brestTotalP$scale
#[1] 0.6381149
...
# What we are going to do now is calculate the sd of the residuals as each of the
# 84 components is added in turn 
model2$tg.sd.newlyn <- vector(mode="numeric", length=84)

for(i in 1:84){
  model2$data.newlyn <- data.frame(msl=model2$newlynTotalP, t=seq(from=1960, to=1999), Pa=model2$Pa[,i])
  model2$tg.lmRob.newlyn <- lmRob(msl ~ t + Pa, data=model2$data.newlyn)
...
# Variance reduction
#> (model2$tg.lmRob.newlynTotalP$scale-model2$tg.lmRob.newlyn$scale)/model2$tg.lmRob.newlynTotalP$scale
#[1] 0.3229579
#> model2$tg.lmRob.newlyn$scale/model2$tg.lmRob.newlynTotalP$scale
#[1] 0.6770421
...
# Calculate over 1953-1999 period

model3<-new.env()

# Just use annual pressures over the periods of the model data (Pa) ending in 2000 (end of model pressure) 
model3$newlynPa <-
  met$hadSLP2rAnnualPressureArray[intersect(which(met$hadSLP2rAnnualTime>=1953), which(met$hadSLP2rAnnualTime<=1999)),1]
model3$brestPa <-
  met$hadSLP2rAnnualPressureArray[intersect(which(met$hadSLP2rAnnualTime>=1953), which(met$hadSLP2rAnnualTime<=1999)),2]

model3$time <- c(1953:1999)
tg_annual$brest5399 <- tg_annual$brest[147:193,]
tg_annual$newlyn5399 <- tg_annual$newlyn[38:84,]

# Calculate the total pressure
model3$brestTotalP <- 1025*9.8*tg_annual$brest5399[,2]/1000 + model3$brestPa
model3$newlynTotalP <- 1025*9.8*tg_annual$newlyn5399[,2]/1000 + model3$newlynPa

model3$tg.lmRob.brestTotalP <- lmRob(model3$brestTotalP ~ model3$time)
model3$tg.lmRob.newlynTotalP <- lmRob(model3$newlynTotalP ~ model3$time)

model3$Pa <- array(NA,dim=c(47,(28*3)))
# Zero lag
for(i in 1:28){
  model3$Pa[,i] <- as.vector(met$hadSLP2rAnnualPressureArray[104:150,met$hadSLP2rUniq[i]])
}
# Lag 1 year
for(i in 29:56){
  model3$Pa[,i] <- as.vector(met$hadSLP2rAnnualPressureArray[103:149,met$hadSLP2rUniq[(i-28)]])
}
# Lag 2 years....
for(i in 57:84){
  model3$Pa[,i] <- as.vector(met$hadSLP2rAnnualPressureArray[102:148,met$hadSLP2rUniq[(i-56)]])
}
...
# What we are going to do now is calculate the sd of the residuals as each of the
# 84 components is added in turn 
model3$tg.sd.brest <- vector(mode="numeric", length=84)

for(i in 1:84){
  model3$data.brest <- data.frame(msl=model3$brestTotalP, t=seq(from=1953,to=1999), Pa=model3$Pa[,i])
  model3$tg.lmRob.brest <- lmRob(msl ~ t + Pa, data=model3$data.brest)
...
model3$tg.lmRob.brest <- lmRob(msl ~ t + Pa1 + Pa2 + Pa3 + Pa4 + Pa5 + Pa6 + Pa7 + Pa8 + Pa9, x=T, y=T,
                               data=model3$data.brest)

# Variance reduction
#> (model3$tg.lmRob.brestTotalP$scale-model3$tg.lmRob.brest$scale)/model3$tg.lmRob.brestTotalP$scale
#[1] 0.4170678 (0.1974664)
#> model3$tg.lmRob.brest$scale/model3$tg.lmRob.brestTotalP$scale
#[1] 0.5829322 (0.8025336)
...
# What we are going to do now is calculate the sd of the residuals as each of the
# 84 components is added in turn 
model3$tg.sd.newlyn <- vector(mode="numeric", length=84)

for(i in 1:84){
  model3$data.newlyn <- data.frame(msl=model3$newlynTotalP, t=seq(from=1953, to=1999), Pa=model3$Pa[,i])
  model3$tg.lmRob.newlyn <- lmRob(msl ~ t + Pa, data=model3$data.newlyn)
...
model3$tg.lmRob.newlyn <- lmRob(msl ~ t + Pa1 + Pa2 + Pa3 + Pa4 + Pa5 + Pa6 + Pa7 + Pa8 + Pa9, x=T, y=T,
                               data=model3$data.newlyn)

# Variance reduction
#> (model3$tg.lmRob.newlynTotalP$scale-model3$tg.lmRob.newlyn$scale)/model3$tg.lmRob.newlynTotalP$scale
#[1] 0.557207 (0.4281596)
#> model3$tg.lmRob.newlyn$scale/model3$tg.lmRob.newlynTotalP$scale
#[1] 0.4427930 (0.5718404)
...
model4$tg.lmRob.brestTotalP <- lmRob(model4$brestTotalP ~ model4$time)
model4$tg.lmRob.newlynTotalP <- lmRob(model4$newlynTotalP ~ model4$time, x=T)

model4$Pa <- array(NA,dim=c(28,(28*3)))
# Zero lag
for(i in 1:28){
...
# Lag 1 year
for(i in 29:56){
  model4$Pa[,i] <- as.vector(met$hadSLP2rAnnualPressureArray[66:93,met$hadSLP2rUniq[(i-28)]])
}
# Lag 2 years....
for(i in 57:84){
  model4$Pa[,i] <- as.vector(met$hadSLP2rAnnualPressureArray[65:92,met$hadSLP2rUniq[(i-56)]])
}
...
# What we are going to do now is calculate the sd of the residuals as each of the
# 84 components is added in turn 
model4$tg.sd.brest <- vector(mode="numeric", length=84)

for(i in 1:84){
  model4$data.brest <- data.frame(msl=model4$brestTotalP, t=seq(from=1916,to=1943), Pa=model4$Pa[,i])
  model4$tg.lmRob.brest <- lmRob(msl ~ t + Pa, data=model4$data.brest)
...
model4$tg.lmRob.brest <- lmRob(msl ~ t + Pa1 + Pa2 + Pa3 + Pa4 + Pa5 + Pa6 + Pa7 + Pa8 + Pa9, x=T, y=T,
                               data=model4$data.brest)

# Variance reduction
#> (model4$tg.lmRob.brestTotalP$scale-model4$tg.lmRob.brest$scale)/model4$tg.lmRob.brestTotalP$scale
#[1] 0.4170678 (0.4769152)
#> model4$tg.lmRob.brest$scale/model4$tg.lmRob.brestTotalP$scale
#[1] 0.5829322 (0.5230848)
...
# What we are going to do now is calculate the sd of the residuals as each of the
# 84 components is added in turn 
model4$tg.sd.newlyn <- vector(mode="numeric", length=84)

for(i in 1:84){
  model4$data.newlyn <- data.frame(msl=model4$newlynTotalP, t=seq(from=1916, to=1943), Pa=model4$Pa[,i])
  model4$tg.lmRob.newlyn <- lmRob(msl ~ t + Pa, data=model4$data.newlyn)
...
model4$tg.lmRob.newlyn <- lmRob(msl ~ t + Pa1 + Pa2 + Pa3 + Pa4 + Pa5 + Pa6 + Pa7 + Pa8 + Pa9, x=T, y=T,
                               data=model4$data.newlyn)

# Variance reduction
#> (model4$tg.lmRob.newlynTotalP$scale-model4$tg.lmRob.newlyn$scale)/model4$tg.lmRob.newlynTotalP$scale
#[1] 0.7737778 (0.5900359)
#> model4$tg.lmRob.newlyn$scale/model4$tg.lmRob.newlynTotalP$scale
#[1] 0.2262222 (0.4099641)
...
# Follow Thompson (1980) and see if we can improve
# Uses pressures from HadSLP2r
# Calculate over 1916-1999 period

model5<-new.env()

# Just use annual pressures over the periods of the model data (Pa) ending in 2000 (end of model pressure) 
model5$newlynPa <-
  met$hadSLP2rAnnualPressureArray[intersect(which(met$hadSLP2rAnnualTime>=1916), which(met$hadSLP2rAnnualTime<=1999)),1]
model5$brestPa <-
  met$hadSLP2rAnnualPressureArray[intersect(which(met$hadSLP2rAnnualTime>=1916), which(met$hadSLP2rAnnualTime<=1999)),2]

model5$time <- c(1916:1999)
tg_annual$brest1699 <- tg_annual$brest[110:193,]
tg_annual$newlyn1699 <- tg_annual$newlyn[1:84,]

# Calculate the total pressure
model5$brestTotalP <- 1025*9.8*tg_annual$brest1699[,2]/1000 + model5$brestPa
model5$newlynTotalP <- 1025*9.8*tg_annual$newlyn1699[,2]/1000 + model5$newlynPa

model5$tg.lmRob.brestTotalP <- lmRob(model5$brestTotalP ~ model5$time)
model5$tg.lmRob.newlynTotalP <- lmRob(model5$newlynTotalP ~ model5$time, x=T)

model5$Pa <- array(NA,dim=c(84,(28*3)))
# Zero lag
for(i in 1:28){
  model5$Pa[,i] <- as.vector(met$hadSLP2rAnnualPressureArray[67:150,met$hadSLP2rUniq[i]])
}
# Lag 1 year
for(i in 29:56){
  model5$Pa[,i] <- as.vector(met$hadSLP2rAnnualPressureArray[66:149,met$hadSLP2rUniq[(i-28)]])
}
# Lag 2 years....
for(i in 57:84){
  model5$Pa[,i] <- as.vector(met$hadSLP2rAnnualPressureArray[65:148,met$hadSLP2rUniq[(i-56)]])
}
...
# What we are going to do now is calculate the sd of the residuals as each of the
# 84 components is added in turn 
model5$tg.sd.brest <- vector(mode="numeric", length=84)

for(i in 1:84){
  model5$data.brest <- data.frame(msl=model5$brestTotalP, t=seq(from=1916,to=1999), Pa=model5$Pa[,i])
  model5$tg.lmRob.brest <- lmRob(msl ~ t + Pa, data=model5$data.brest)
...
model5$tg.lmRob.brest <- lmRob(msl ~ t + Pa1 + Pa2 + Pa3 + Pa4 + Pa5 + Pa6 + Pa7 + Pa8 + Pa9, x=T, y=T,
                               data=model5$data.brest)

# Variance reduction
#> (model5$tg.lmRob.brestTotalP$scale-model5$tg.lmRob.brest$scale)/model5$tg.lmRob.brestTotalP$scale
#[1] 0.3253409
#> model5$tg.lmRob.brest$scale/model5$tg.lmRob.brestTotalP$scale
#[1] 0.674659
...
# What we are going to do now is calculate the sd of the residuals as each of the
# 84 components is added in turn 
model5$tg.sd.newlyn <- vector(mode="numeric", length=84)

for(i in 1:84){
  model5$data.newlyn <- data.frame(msl=model5$newlynTotalP, t=seq(from=1916, to=1999), Pa=model5$Pa[,i])
  model5$tg.lmRob.newlyn <- lmRob(msl ~ t + Pa, data=model5$data.newlyn)
...
model5$tg.lmRob.newlyn <- lmRob(msl ~ t + Pa1 + Pa2 + Pa3 + Pa4 + Pa5 + Pa6 + Pa7 + Pa8 + Pa9, x=T, y=T,
                               data=model5$data.newlyn)

# Variance reduction
#> (model5$tg.lmRob.newlynTotalP$scale-model5$tg.lmRob.newlyn$scale)/model5$tg.lmRob.newlynTotalP$scale
#[1] 0.3732652
#> model5$tg.lmRob.newlyn$scale/model5$tg.lmRob.newlynTotalP$scale
#[1] 0.6267348
