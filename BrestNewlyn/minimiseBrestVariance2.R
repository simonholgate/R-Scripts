...
most.corr.pa <- function(corr.array, slp.array, start.year, end.year, years){
  ...
  most.correlated <- tmp[most.correlated.point,ps:pe] 
...
brestx.corr.dt <- corr.pressure.dt(brestx.reg.tot.p,
                                            slpNAtlAnnArray,
                                            brestx.reg.start.year,
                                            brestx.reg.end.year, slp.yrs)
...
for (offset in offset.low:offset.high) {
  ...
  if (offset==13) {
    brestx.pred.tot.p.13 <- brestx.pred.tot.p
  }
...
###############################################################################
## Having found the optimal offset of 13mm for 1916-1943, now carry out
## regression on the entire, modified time-series
###############################################################################
brestx.mod.tot.p.16 <- vector(length=length(1916:1943), mode='numeric')
## Get the sl data over the period 1916-1943
##brestx.pred16.sl <- offset.brest.tg.data(brestx.pred.start.year,
##                                    brestx.pred71.end.year, 13)
## Convert sl into total pressure
...
data.mod.brestx.16 <- data.frame(msl=brestx.mod.tot.p.16,
                                  t=seq(from=brestx.pred.start.year,
                                    to=brestx.pred.end.year),
                                  Pa=slpNAtlAnnArray[43,17,46:73])
## Build the model
...
brestx.pred71.most.corr.pa <- slpNAtlAnnArray[43,17,1:73]
...
for (offset in offset.low:offset.high) {
  ...
  brestx.pred71.tot.p <- total.pressure(brestx.pred71.sl$Height,
                                        brestx.pred71.pa)
  if (offset==18) {
    brestx.pred71.tot.p.18 <- brestx.pred71.tot.p
  }
  if (offset==13) {
    brestx.pred71.tot.p.13 <- brestx.pred71.tot.p
  }
  ## Difference reduction - subtract the prediction from the offset data
  ## and calculate the rms of the residual
...
brestx.clean[1:73] <- brestx.clean[1:73] +
  (brestx.pred71.tot.p.18 - tg.lmRob.brestx.pred71$fit)
...
save(file="Sam_data.RData", brestx.mod.tot.p, brestx.most.corr.pa)
...
(var(detrend(brestx.pred.tot.p.13)) -
 var(brestx.pred.tot.p.13 - tg.lmRob.brestx.pred$fit))/
  var(detrend(brestx.pred.tot.p.13)) * 100
##[1] 1.562719

(rms(detrend(brestx.pred.tot.p.13)) -
 rms(brestx.pred.tot.p.13 - tg.lmRob.brestx.pred$fit))/
  rms(detrend(brestx.pred.tot.p.13)) * 100
##[1] 0.7893907

(var(detrend(brestx.mod.tot.p.16)) -
 var(brestx.mod.tot.p.16 - tg.lmRob.mod.brestx.16$fitted))/
  var(detrend(brestx.mod.tot.p.16)) * 100
##[1] 21.22722
...
lines(tg.lmRob.brestx$x[,2],tg.lmRob.brestx$fitted/cf  - 9900, col='red', lwd=2)
lines(brestx.pred.start.year:brestx.pred.end.year,
      brestx.pred.tot.p.13/cf - 9900, col='magenta', lwd=2)
lines(brestx.pred.start.year:brestx.pred.end.year,
      tg.lmRob.brestx.pred$fit/cf - 9900, col='orange', lwd=2)
grid(lwd=2)
#dev.off()
...
lines(brestx.pred71.start.year:brestx.pred71.end.year,
      brestx.pred71.tot.p.18/cf  - 9900, col='magenta', lwd=2)
#lines(brestx.pred71.start.year:brestx.pred71.end.year,
#      brestx.pred71.tot.p.13, col='green')
lines(brestx.pred71.start.year:brestx.pred71.end.year,
...
plot(c(offset.low:offset.high), rms.pred.1643, type='l', xlab='Offset [mm]',
     ylab='RMS of Offset Data minus Prediction', lwd=2, ylim=c(300,500))
grid(lwd=2)
...
lines(brestx.pred71.start.year:brestx.reg.end.year, brestx.clean/cf  - 9900,
      col='red', lwd=2)
lines(brestx.pred71.start.year:brestx.pred71.end.year,
      brestx.pred71.tot.p.18/cf  - 9900, col='blue', lwd=2)
