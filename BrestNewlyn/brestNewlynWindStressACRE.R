...
windEAnnualMean <- array(NA, dim=c(l, m, numYears))
windNAnnualMean <- array(NA, dim=c(l, m, numYears))

for (i in 1:numYears){
  junk <- readBin(inWindEAnnualMean, "integer", size=4, n = 1)
  junkw <- readBin(inWindEAnnualMean, "numeric", size=8, n = lm)
  dim(junkw) <- c(l,m)
  windEAnnualMean[,,i] <- mirror.matrix(junkw)
  junk <- readBin(inWindEAnnualMean, "integer", size=4, n = 1)
}
for (i in 1:numYears){
  ...
  dim(junkw) <- c(l,m)
  windNAnnualMean[,,i] <- mirror.matrix(junkw)
  junk <- readBin(inWindNAnnualMean, "integer", size=4, n = 1)
}
...
x11()
filled.contour(lon,(-1*lat),windEAnnualMean[,,1], color=tim.colors, nlevels=20,
 plot.axes = { world(shift=T, add=T) }, zlim=c(-0.12, 0.15))

x11()
filled.contour(lon,(-1*lat),windNAnnualMean[,,1], color=tim.colors, nlevels=20,
 plot.axes = { world(shift=T, add=T) }, zlim=c(-0.12, 0.15))
...
save(file="~/data/ACRE/wsACRE.RData", wsEAnnualMean, windEAnnualMean, wsNAnnualMean, windNAnnualMean, wslon, lon, lat)
