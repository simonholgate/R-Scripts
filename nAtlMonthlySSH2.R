...
# Script assumes nAtlSSH.R is run first
...
months <- seq(from=(1950+1/24), to=(1999+(11/12+1/24)), by=1/12)
nmon <- nyr*12

fileName <- "~/data/vassil/ssh/ssh_50-00av.dat"
# Open binary file for reading
inConn <- file(fileName, "rb")

# Set up main sea surface height array
sshMonthly <- array(NA,dim=c(xn,yn,nmon))
...
for (i in 1:xn){
  for (j in 1:yn){
    sshMonthly[i,j,which(sshMonthly[i,j,]<=-1e29)]<-NA
# Convert to mm
    sshMonthly[i,j,] <- sshMonthly[i,j,]*1000
  }
}
...
lenW <- length(mask_w[,1])
lenE <- length(mask_e[,1])
lenLats <- length(lats)

masked_w_atl_monthly <- array(NA, dim=c(lenW,nmon))
masked_e_atl_monthly <- array(NA, dim=c(lenE,nmon))

for (i in 1:nmon){
  for (j in 1:lenE){
    masked_e_atl_monthly[j,i] <- sshMonthly[mask_e[j,1],mask_e[j,2],i]
  }
  for (j in 1:lenW){
    masked_w_atl_monthly[j,i] <- sshMonthly[mask_w[j,1],mask_w[j,2],i]
  }
}

masked_c_atl_monthly <- sshMonthly[300,,]

# Remove linear trend from each station
for (i in 1:lenLats){
  if(length(which(is.finite(masked_c_atl_monthly[i,])))>0){
    junk <- lm(masked_c_atl_monthly[i,] ~ months)
    masked_c_atl_monthly[i,] <- junk$resid
  }
}
for (i in 1:lenE){
  if(length(which(is.finite(masked_e_atl_monthly[i,])))>0){
    junk <- lm(masked_e_atl_monthly[i,] ~ months)
    masked_e_atl_monthly[i,] <- junk$resid
  }
}
for (i in 1:lenW){
  if(length(which(is.finite(masked_w_atl_monthly[i,])))>0){
    junk <- lm(masked_w_atl_monthly[i,] ~ months)
    masked_w_atl_monthly[i,] <- junk$resid
  }
}

# Remove 1960-1990 mean annual cycle from data to show the anomaly
start <- which(months==(1960+1/24))
finish <-
  intersect(which(months>=(1989+(11/12+1/24))),which(months<1990))
monthsIndices <- c(start:finish)

arrayWMonthly <- masked_w_atl_monthly[,monthsIndices]
arrayEMonthly <- masked_e_atl_monthly[,monthsIndices]
arrayCMonthly <- masked_c_atl_monthly[,monthsIndices]

meansArrayWMonthly <- array(NA,dim=c(lenW,12))
meansArrayEMonthly <- array(NA,dim=c(lenE,12))
meansArrayCMonthly <- array(NA,dim=c(lenLats,12))

for (j in 1:lenE){
  junk <- arrayEMonthly[j,]
  dim(junk) <- c(12,30)
  meansArrayEMonthly[j,] <- rowMeans(junk)
}
for (j in 1:lenW){
  junk <- arrayWMonthly[j,]
  dim(junk) <- c(12,30)
  meansArrayWMonthly[j,] <- rowMeans(junk)
}
for (j in 1:lenLats){
  junk <- arrayCMonthly[j,]
  dim(junk) <- c(12,30)
  meansArrayCMonthly[j,] <- rowMeans(junk)
}

dim(masked_w_atl_monthly) <- c(lenW,12,nyr)
dim(masked_e_atl_monthly) <- c(lenE,12,nyr)
dim(masked_c_atl_monthly) <- c(lenLats,12,nyr)

anom_w_atl_monthly <- array(NA, dim=dim(masked_w_atl_monthly))
anom_e_atl_monthly <- array(NA, dim=dim(masked_e_atl_monthly))
anom_c_atl_monthly <- array(NA, dim=dim(masked_c_atl_monthly))

#dim(anom_w_atl_monthly) <- c(lenW,12,nyr)
#dim(anom_e_atl_monthly) <- c(lenE,12,nyr)

for (j in 1:nyr){
  for (i in 1:lenW){
   anom_w_atl_monthly[i,,j] <- 
     masked_w_atl_monthly[i,,j] - meansArrayWMonthly[i,]
  }
  for (i in 1:lenE){
   anom_e_atl_monthly[i,,j] <- 
     masked_e_atl_monthly[i,,j] - meansArrayEMonthly[i,]
  }
  for (i in 1:lenLats){
   anom_c_atl_monthly[i,,j] <- 
     masked_c_atl_monthly[i,,j] - meansArrayCMonthly[i,]
  }
}

dim(anom_w_atl_monthly) <- c(lenW,nmon)
dim(anom_e_atl_monthly) <- c(lenE,nmon)
dim(anom_c_atl_monthly) <- c(lenLats,nmon)
#
...
postscript(file="eAtlMonthlyAnom30_43DT.ps")
op <- par()
par(oma=c( 0,0,3,5), family="HersheySans")
image(months, e3043, 
  rotate90.matrix(mirror.matrix(anom_e_atl_monthly[e3043,])), 
...
par <- op
title(main="Model Monthly DT Anomaly: East side of Atlantic 30-43N")
dev.off()

postscript(file="wAtlMonthlyAnom30_43DT.ps")
par(oma=c( 0,0,3,5), family="HersheySans")
image(months, w3043, 
  rotate90.matrix(mirror.matrix(anom_w_atl_monthly[w3043,])), 
  col=tim.colors(20), ann=FALSE, axes=FALSE,
  zlim=c(-100,120))
...
image.plot(legend.only=TRUE, zlim=c(-100,120), legend.lab="mm/yr")
par <- op
title(main="Model Monthly DT Anomaly: West side of Atlantic 30-43N")
dev.off()

postscript(file="cAtlMonthlyAnom30_43DT.ps")
par(oma=c( 0,0,3,5), family="HersheySans")
image(months, c3043, 
  rotate90.matrix(mirror.matrix(anom_c_atl_monthly[c3043,])), 
  col=tim.colors(20), ann=FALSE, axes=FALSE,
  zlim=c(-100,120))
axis(2,at=c(300,310,320,330,340,350,360),
  labels=c("30.95", "32.94", "34.89", "36.79", "38.64", "40.45", "42.21"))
axis(1,at=c(1950, 1960,1970,1980,1990,2000),
  labels=c("1950","1960", "1970", "1980", "1990", "2000"))
title(ylab="Latitude", xlab="Year")
par(oma=c( 0,0,0,1))
image.plot(legend.only=TRUE, zlim=c(-100,120), legend.lab="mm/yr")
par <- op
title(main="Model Monthly DT Anomaly: Central Atlantic 30-43N")
dev.off()
