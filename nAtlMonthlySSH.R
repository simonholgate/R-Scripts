...
postscript(file="eAtlMonthlyAnom30_43.ps")
op <- par()
par(oma=c( 0,0,3,5))
image(months, e3043, 
  rotate270.matrix(anom_e_atl_monthly[e3043,]), 
...
par <- op
title(main="Model Monthly Anomaly: East side of Atlantic 30-43N")
dev.off()

postscript(file="wAtlMonthlyAnom30_43.ps")
par(oma=c( 0,0,3,5))
image(months, w3043, 
  rotate270.matrix(anom_w_atl_monthly[w3043,]), 
  col=tim.colors(20), ann=FALSE, axes=FALSE,
  zlim=c(-100,100))
...
image.plot(legend.only=TRUE, zlim=c(-100,100), legend.lab="mm/yr")
par <- op
title(main="Model Monthly Anomaly: West side of Atlantic 30-43N")
dev.off()
