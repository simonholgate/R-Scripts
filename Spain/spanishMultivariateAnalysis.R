...
###############################################################################
model<-new.env()
load("~/diskx/polcoms/spain/spanishModelMonthlyMean.RData", envir=model)
...
model$time <- seq.Date(from=as.Date("1960/1/15"), to=as.Date("1999/12/15"), by="1 month")
model$t <- seq(from=0,to=((40*12)-1))

# Generate annual means from model data
model$annualYrs <- seq.Date(from=as.Date("1960/7/1"), to=as.Date("1999/7/1"), by="1 year")
model$annualMeans <- array(NA,dim=c(40,12))
for (i in 1:12){
  model$tmp <- model$spanishMonthlyMean[,i]
  dim(model$tmp) <- c(12,40)
...
model$tenYrTrends <- array(NA,dim=c((40-9),12))
model$midpointYrs <- seq.Date(from=as.Date("1965/1/1"), length=(40-9), by="1 year")
for (i in 1:(40-9)) {
  for ( j in 1:12) {
