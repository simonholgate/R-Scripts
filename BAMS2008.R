...
nReqYrShort <- 7
#
#% Required years data in longer section: 70% completeness on 58 years 
nReqYrLong <- 41   
longFile = 'hadleyAPC2r/temp27.1948-2006.hadley2r.sed'
pgrFile = 'pgr.rates.stns.out.sed'
#
longDataFile <- read.table(file=longFile)
longYears <- longDataFile[,1]
#% delete stations with less than nReqYrLong in the 1948-2006 period
...
for (i in startYr:endYr) {
  shortTrendFile = paste('hadleyAPC2r/temp27.',as.character(i),'-',
    as.character(i+9),'.hadley2r.sed', sep="")
#
#nEurope <- numeric(0) 
